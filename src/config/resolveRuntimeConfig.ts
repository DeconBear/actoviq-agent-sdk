...
const LEGACY_ENV_KEYS = {
  ...
  defaultSonnetModel: ['ANTH', 'ROPIC_DEFAULT_medium_MODEL'].join(''),
  defaultOpusModel: ['ANTH', 'ROPIC_DEFAULT_max_MODEL'].join(''),
  defaultHaikuModel: ['ANTH', 'ROPIC_DEFAULT_min_MODEL'].join(''),
} as const;

const FALLBACK_MODEL = ['cl', 'aude-medium-4-5-20250929'].join('');
...
export async function resolveRuntimeConfig(
  ...
  const model =
    ...
    getConfigValue(
      envFromLoadedConfig,
      'ACTOVIQ_DEFAULT_medium_MODEL',
      LEGACY_ENV_KEYS.defaultSonnetModel,
    ) ??
    getConfigValue(
      envFromLoadedConfig,
      'ACTOVIQ_DEFAULT_max_MODEL',
      LEGACY_ENV_KEYS.defaultOpusModel,
    ) ??
    getConfigValue(
      envFromLoadedConfig,
      'ACTOVIQ_DEFAULT_min_MODEL',
