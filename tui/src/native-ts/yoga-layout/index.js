...
forceWidth = false, forceHeight = false) {
    ...
    if (node.measureFunc && node.children.length === 0) {
        const innerW = wMode === MeasureMode.Undefined
            ? NaN
            : Math.max(0, width - paddingBorderWidth);
        const innerH = hMode === MeasureMode.Undefined
            ? NaN
    ...
    for (let li = 0; li < lineCount; li++) {
        ...
        for (const c of line) {
            ...
            const hasCrossAutoMargin = c._hasAutoMargin &&
            ...
            else if (childAlign === Align.Stretch &&
                !hasCrossAutoMargin &&
                !isWrap &&
    ...
    const finalMainSize = mainMode === MeasureMode.Exactly
        ? mainSize
    ...
    const finalCrossSize = crossMode === MeasureMode.Exactly
        ? crossSize
    ...
    for (let li = 0; li < lineCount; li++) {
        ...
        const autoMarginMainSize = numAutoMarginsMain > 0 && remainingMain > 0
...
function computeFlexBasis(child, mainAxis, availableMain, availableCross, crossMode, ownerWidth, ownerHeight) {
    ...
    let crossConstraintMode = isDefined(crossConstraint)
        ? MeasureMode.Exactly
...
function resolveFlexibleLengths(children, availableInnerMain, totalFlexBasis, isMainRow, ownerW, ownerH) {
    ...
    for (let i = 0; i < n; i++) {
        ...
        const inflexible = !isDefined(availableInnerMain) ||
...
function isStretchAlign(child) {
    ...
    if (!p)
        return false;
    const align = child.style.alignSelf === Align.Auto
        ? p.style.alignItems
